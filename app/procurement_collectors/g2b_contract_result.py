...
class G2bContractResultCollector(ProcurementCollector):
    ...
    def collect(self) -> ProcurementCollectResult:
        ...
        openapi = (self.config.get("openapi") or {}).get("contract_result") or {}
        service_path = str(openapi.get("service_path") or "")
        list_op = str(openapi.get("list_operation") or "getCntrctInfoListServc")
        search_op = str(openapi.get("search_operation") or "getCntrctInfoListServcPPSSrch")
        ...
        for kw in keywords:
            ...
            for row in rows:
                ...
                key = str(row.get("untyCntrctNo") or row.get("cntrctNo") or "")
                if not key:
                    key = f"{row.get('bidNtceNo')}:{row.get('cntrctNm')}"
        ...
        if not by_key:
            ...
            for row in rows:
                ...
                key = str(row.get("untyCntrctNo") or "")
                if key and key not in by_key:
                    by_key[key] = stamped

        records = list(by_key.values())[: self.max_records]
        out_path = Path(self.raw_dir) / "g2b_contract_results.jsonl"
