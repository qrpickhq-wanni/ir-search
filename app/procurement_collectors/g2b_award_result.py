...
from app.procurement_collectors.g2b_bid_notice import _all_keywords, _notice_key
...
class G2bAwardResultCollector(ProcurementCollector):
    ...
    def collect(self) -> ProcurementCollectResult:
        ...
        openapi = (self.config.get("openapi") or {}).get("award_result") or {}
        service_path = str(openapi.get("service_path") or "")
        list_op = str(openapi.get("list_operation") or "getScsbidListSttusServc")
        search_op = str(openapi.get("search_operation") or "getScsbidListSttusServcPPSSrch")
        windows = self.config.get("windows") or {}
        bgn, end = self.date_window(int(windows.get("award_contract_past_days", 548)))
        ...
        for kw in keywords:
            ...
            for row in rows:
                ...
                key = _notice_key(row) or str(row.get("bidwinnrNm") or "") + str(
                    row.get("sucsfbidAmt") or ""
                )
        ...
        out_path = Path(self.raw_dir) / "g2b_award_results.jsonl"
